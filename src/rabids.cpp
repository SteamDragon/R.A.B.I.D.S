...
{
    auto& cmdMap = Command::all();
    auto foundCommand = cmdMap.find(interaction.data.name);
    if (foundCommand == cmdMap.end())
    {
...
ServerStatus RABIDS::checkServerStatus()
{
    ...
    std::string fullCommand = std::accumulate(std::next(v.begin()), v.end(),
                                                v[0], space);
    std::string results = exec(("ps -ax | grep \"" + fullCommand + "\" | wc -l").c_str());
